...
void load_net(Net& net, bool is_cuda)
{
    ...
    /*auto result = readNetFromONNX("C:\\Users\\edg\\Downloads\\detr_resnet50.onnx");*/
    if (is_cuda)
    {
...
void detect(Mat& image, Net& net)
{
    ...
    for (unsigned long i = 0; i < nms_result.size(); ++i)
    {
        ...
        cv::rectangle(modelInput, topLeft/640, bottomRight/640, Scalar(0, 255, 0), 3);
        putText(modelInput, to_string(int(confidences[idx] * 100)) + "%" + classes[classIds[idx]], Point(boxes[idx].x, boxes[idx].y)/640, 1, 3, Scalar(0, 255, 255), 2);
    }
